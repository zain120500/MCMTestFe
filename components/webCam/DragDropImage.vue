<template>
    <div class="text-xs w-full sm:w-1/3 mt-3 text-white">
        <div v-if="file != null" class="mt-2">
            <div class="image-container border-2 border-slate-400 rounded-lg">
                ...
                <div class="absolute flex justify-end items-end bottom-0 right-0 left-0 p-1">
                    <button class="bg-white rounded-md p-2 border text-black" @click="showModal">
                        <span clas="">View</span>
                    </button>
                </div>
            </div>
        </div>
        <div v-else class="dropzone-container grid grid-rows-2 text-white" :style="isDragging == true ? 'border-color: green;' : ''"
            @dragover="dragover" @dragleave="dragleave" @drop="drop">
            <div>
                <div>
                    <div v-if="isDragging">Drop Foto.</div>
                    <div v-else>Drag Foto</div>
                </div>
            </div>
            <div class="">
                <input type="file" name="file" :id="id" class="hidden" @change="onChange" ref="file"
                    accept="application/octet-stream .jpg, .jpeg, .png" />
                <label class="btn btn-accent btn-sm text-white file-label mr-2" :for="id">
                    <Icon name="ion:image-sharp" class="mr-1"></Icon> Browse
                </label>
                <button class="btn btn-accent btn-sm text-white " @click="showModalWebCam">
                    <Icon name="bi:camera-fill" class="mr-1"></Icon> Camera
                </button>
            </div>
        </div>
